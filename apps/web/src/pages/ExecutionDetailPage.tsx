...
export default function ExecutionDetailPage() {
  ...
  return (
    <div className="space-y-6">
      <Link to="/executions" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700">
        ← Back to executions
      </Link>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-bold text-slate-900">{ex.workflow_name || "Execution"}</h2>
              <StatusBadge status={ex.status} />
            </div>
            <p className="mt-1 font-mono text-xs text-slate-400">Execution #{ex.id}</p>
          </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="card p-5">
          <h3 className="mb-3 font-semibold text-slate-800">Input payload</h3>
          <JsonBlock value={ex.input_payload} />
        </div>
        <div className="card p-5">
          <h3 className="mb-3 font-semibold text-slate-800">Final output</h3>
      ...
      <div className="card">
        <div className="border-b border-slate-100 px-6 py-4">
          <h3 className="font-semibold text-slate-800">Step-by-step logs</h3>
          <p className="text-xs text-slate-400">{logs.length} steps executed</p>
        </div>
        <ol className="divide-y divide-slate-100">
          ...
          {logs.length === 0 && (
            <li className="px-6 py-8 text-center text-sm text-slate-400">No step logs recorded.</li>
          )}
        </ol>
      </div>
    </div>
  ...
  return (
    <li className="px-6 py-4">
      <div className="flex items-center gap-3">
        ...
        <span className="min-w-0 flex-1 truncate text-sm font-medium text-slate-800">{log.step_name}</span>
